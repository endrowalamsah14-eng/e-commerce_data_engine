...
DB_HOST = "127.0.0.1"
DB_PORT = "5433" # Port dibedain buat Shard 2
DB_NAME = "emarkrtz_shard_02"
...
def generate_regular_shard_02(target_rows=1000000, batch_size=5000):
    conn = get_connection()
    cur = conn.cursor()
    now = datetime.now()
    ...
    execute_values(cur, "INSERT INTO stores (store_id, store_name, tier, created_at) VALUES %s", [(store_id, "Toko Baju Standar", "Basic", now)])
    
    products = [(uuid.uuid4().hex, store_id, f"SKU-REG-{i}", "Baju Polos", 75000.0, False) for i in range(50)]
    ...
    customers = [(uuid.uuid4().hex, store_id, f"guest_{i}@email.com", "guest") for i in range(1000)]
    execute_values(cur, "INSERT INTO customers (customer_id, store_id, email, segment) VALUES %s", customers)
    conn.commit()
    print("Master Data Shard 02 Siap!")
        ...
        for i in range(0, target_rows, batch_size):
            ...
            for _ in range(batch_size):
                ...
                for _ in range(num_items):
                    ...
                    order_lines_data.append((uuid.uuid4().hex, order_id, store_id, chosen_product, qty, 75000.0, 75000.0))
            ...
            conn.commit()
            print(f"[Shard 02] {i + batch_size} Pesanan normal mendarat.")
    except Exception as e:
        print(f"Error: {e}")

if __name__ == "__main__":
    generate_regular_shard_02(target_rows=1000000, batch_size=5000)
